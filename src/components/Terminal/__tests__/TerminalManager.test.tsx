...
jest.mock('../Terminal', () => {
  return function MockTerminal({ ipAddress }: { ipAddress: string }) {
    return <div data-testid="mock-terminal" data-ip={ipAddress}>Mock Terminal Content</div>;
  };
});
...
jest.mock('../ThemeSelector', () => {
  return function MockThemeSelector({ minimal }: { minimal?: boolean }) {
    return <div data-testid="mock-theme-selector" data-minimal={minimal ? 'true' : 'false'}>Theme Selector</div>;
  };
});
...
describe('TerminalManager', () => {
  ...
  test('renders with default session when no initialSessions provided', () => {
    ...
    expect(terminalElement).toHaveAttribute('data-ip', expect.stringMatching(/^192\.168\.\d+\.\d+$/));
  ...
  test('does not render the "New Terminal" button when maxSessions is reached', () => {
    ...
    render(<TerminalManager initialSessions={initialSessions} maxSessions={2} />);

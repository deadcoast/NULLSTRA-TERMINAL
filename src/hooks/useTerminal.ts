...
import { formatTimestamp, getFileSystemItem } from '../utils/terminalCommands/helpers';
import commandRegistry, { CommandContext, FileSystem, FileSystemItem, TerminalMessage } from '../utils/terminalCommands/index';
...
const useTerminal = (initialMessages: TerminalMessage[] = []): UseTerminalReturnType => {
  const [messages, setMessages] = useState<TerminalMessage[]>(initialMessages);
  // Use useState for context elements that change
  const [currentPath, setCurrentPath] = useState<string>(initialContextValues.currentPath);
  const [fileSystem, setFileSystem] = useState<FileSystem>(initialContextValues.fileSystem);
  const [environmentVariables, setEnvironmentVariables] = useState<Record<string, string>>(initialContextValues.environmentVariables);
  const [lastExitCode, setLastExitCode] = useState<number>(initialContextValues.lastExitCode);
  ...
  const addMessage = useCallback((messageOrMessages: TerminalMessage | TerminalMessage[]) => {
  ...
  const processErrorMessage = useCallback((message: TerminalMessage): TerminalMessage => {
  ...
  const executeCommand = useCallback(async (commandString: string) => {
    ...
    const command = commandRegistry[commandName] || commandRegistry[Object.keys(commandRegistry).find(key => commandRegistry[key].aliases?.includes(commandName)) || ''];
  ...
  }, [currentPath, fileSystem, commandHistory, environmentVariables, lastExitCode, addMessage, updatePath, getTimestamp, processErrorMessage]);
  ...
  const extractFileList = (fileSystem: FileSystem, currentPath: string): string[] => {

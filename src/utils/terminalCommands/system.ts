  ...
  executor: (args, _context) => {
    ...
    const commands = Object.values(registry)
      ...
      .filter(
        (cmd, index, self) =>
          self.findIndex(
            (c) => (c as Command).name === (cmd as Command).name,
          ) === index,
      );
...
const echoCommand: Command = {
  ...
  executor: (args, _context) => {
    const { environmentVariables } = _context;
...
const execCommand: Command = {
  ...
  executor: (args, _context) => {
    const { currentPath, fileSystem } = _context;

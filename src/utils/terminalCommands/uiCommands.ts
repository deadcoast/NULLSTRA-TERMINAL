import { createErrorMessage, createInfoMessage, createSuccessMessage } from './helpers';
...
export const registerUICommands = (registry: CommandRegistry): void => {
  ...
  const themeCommand: Command = {
    ...
    executor: (args, context) => {
      ...
      if (args.length === 0) {
        return createInfoMessage('Opening theme selector...', { animated: true });
      }
      
      if (args[0] === 'list') {
        return createInfoMessage('Available themes: cyberpunk, hacker, midnight, retro', {
          animated: true
        });
      }
      ...
      return createErrorMessage(`Unknown theme: ${selectedTheme}. Try 'theme list' to see available themes.`, {
        animated: true
      });
    },
    aliases: ['themes']
  ...
  const viewCommand: Command = {
    ...
    executor: (args, context) => {
      if (args.length === 0) {
        return createErrorMessage('Please specify a file to view', { animated: true });
      }
      
      // This is handled by Terminal.tsx to show the FileViewer UI
      const fileName = args[0];
      return createInfoMessage(`Opening ${fileName} in viewer...`, { animated: true });
    },
    aliases: []
  };
  ...
  const tableCommand: Command = {
    ...
    usage: 'table <type>',
    examples: ['table system', 'table network', 'table users', 'table processes'],
    executor: (args, context) => {
      // This is intercepted by Terminal.tsx to render tables
      const tableType = args[0] || 'system';
      return createInfoMessage(`Displaying ${tableType} data in table format...`, { animated: true });
    },
    aliases: []
  };
  ...
  const statsCommand: Command = {
    ...
    executor: (args, context) => {
      ...
      return createInfoMessage(`Showing ${component} statistics...`, { animated: true });
    },
    aliases: ['statistics']
  };
  ...
  const taskCommand: Command = {
    ...
    executor: (args, context) => {
      ...
      return createInfoMessage(`Starting ${taskType} task...`, { animated: true });
    },
    aliases: []
  };
  ...
  const downloadCommand: Command = {
    ...
    executor: (args, context) => {
      ...
      return createInfoMessage(`Downloading ${fileName}...`, { animated: true });
    },
    aliases: []
  };

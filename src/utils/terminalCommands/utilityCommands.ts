...
const historyCommand: Command = {
  ...
  executor: (args, context) => {
    ...
    const count = args.length > 0 ? parseInt(args[0], 10) : commandHistory.length;
    ...
    const messages = [
      createInfoMessage('Command history:')
    ];
...
const aliasCommand: Command = {
  ...
  executor: (args, context) => {
    ...
    if (args.length === 0) {
      ...
      const messages = [
        createInfoMessage('Defined aliases:')
      ];
    ...
    if (!aliasMatch) {
      return createErrorMessage('Invalid alias syntax. Usage: alias name="command"');
    }
...
const findCommand: Command = {
  ...
  examples: [
    'find',
    'find /Legislation -name "Security"',
    'find . -type f'
  ],
  executor: (args, context) => {
    ...
    const path = args.length > 0 && !args[0].startsWith('-') ? args[0] : currentPath || '/';
...
const grepCommand: Command = {
  ...
  executor: (args, context) => {
    if (args.length === 0) {
      return createErrorMessage('No pattern specified. Usage: grep pattern [file...]');
    }
    ...
    if (files.length === 0) {
      return createErrorMessage('No files specified. Usage: grep pattern [file...]');
    }
    
    // Simple grep implementation - for demo purposes
    return [
      createInfoMessage(`Searching for "${pattern}" in ${files.join(', ')}...`, {
        animated: true
      }),
      createInfoMessage(`${files[0]}:42: match found: "This is an ${pattern} in the file"`, {
        animated: true
      }),
      createInfoMessage(`${files[0]}:78: match found: "Another ${pattern} appears here"`, {
        animated: true
      }),
...
const dfCommand: Command = {
  ...
  executor: (args, context) => {
    ...
    return [
      createInfoMessage('Filesystem      1K-blocks       Used    Available Use% Mounted on'),
      createInfoMessage('/dev/sda1       125829120000 81788928000 44040192000  65% /'),
      createInfoMessage('/dev/sdb1       52428800000 15728640000 36700160000  30% /data'),
      createInfoMessage('quantum-storage 10995116277760 2528776742912 8466339534848  23% /quantum')
    ];
  }
};

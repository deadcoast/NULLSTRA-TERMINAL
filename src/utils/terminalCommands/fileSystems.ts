...
import { Command, CommandRegistry, FileSystem } from "./types";
...
const lsCommand: Command = {
  ...
  usage: "ls [directory]",
  examples: ["ls", "ls /Command_History", "ls ../Legislation"],
...
const cdCommand: Command = {
  ...
  usage: "cd [directory]",
  examples: ["cd", "cd Security", "cd ..", "cd /Legislation/Security"],
  executor: (args, context) => {
...
const mkdirCommand: Command = {
  ...
  examples: ["mkdir new_folder", "mkdir /Projects/new_project"],
  executor: (args, context) => {
    const { currentPath, fileSystem } = context;

    if (args.length === 0) {
      return createErrorMessage(
        "No directory name specified. Usage: mkdir <directory>",
      );
    }
...
const touchCommand: Command = {
  ...
  examples: ["touch notes.txt", "touch /Projects/readme.md"],
  executor: (args, context) => {
    const { currentPath, fileSystem } = context;

    if (args.length === 0) {
      return createErrorMessage(
        "No filename specified. Usage: touch <filename>",
      );
    }
